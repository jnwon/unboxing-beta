        ...
        this.ref.orderByChild('timestamp').endBefore(this.lastTimestampOfNoti).once("value", async (snapshot) => {
          if(snapshot.exists()){
            ...
            snapshot.forEach((data) => {
              ...
              notis.push({commentId: data.key, ownerId: data.val().ownerId, postId: data.val().postId, postTitle: data.val().postTitle, comment: data.val().comment, followerId: data.val().followerId, followerName: data.val().followerName, timestamp: data.val().timestamp, read: data.val().read});
              this.unread++;
            })
